...
public class Main {
    ...
    private static void entrenarArtista() {
        ...
        // Verificar que no este contratado en alguna cancion
        for (Cancion c : canciones) {
            for (Asignacion a : c.getAsignaciones()) {
                if (a.getArtista().equals(artista)) {
                    System.out.println("No se puede entrenar: el artista ya fue contratado en alguna cancion.");
                    return;
                }
            }
        }
        ...
        if (artista.getRolesHistoricos().contains(rol) || artista.getRolesEntrenados().contains(rol)) {
        ...
        try {
            ...
            System.out.println("No se puede entrenar: " + ex.getMessage());
            return;
        }
    ...
    private static Recital crearRecitalActual() {
        ...
        r.getCanciones().addAll(canciones);
        r.getArtistasBase().addAll(artistasBase);
        return r;
    }
